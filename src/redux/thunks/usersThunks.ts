...
import {usersActions, UserType} from '../reducers/usersReducer';
...
const follow = createAppAsyncThunk<{ userId: number }, number>(
    'users/follow',
...
const unfollow = createAppAsyncThunk<{ userId: number }, number>(
    'users/unfollow',
    async (userId, thunkAPI) => {
        const {dispatch, rejectWithValue} = thunkAPI
        return thunkTryCatch(thunkAPI, async () => {
...
const fetchUserCountForPagination =
    createAppAsyncThunk<{ users: UserType[], currentPage: number }, { pageNumber: number, pageSize: number }>(
        'users/fetchUserCountForPagination',
        async (data, thunkAPI) => {
            const {dispatch} = thunkAPI
            return thunkTryCatch(thunkAPI, async () => {
